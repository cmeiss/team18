...
var EditDescription_1 = require("./EditDescription");
function EditTask(edit) {
    ...
    var id = react_1.useState(edit.task.id)[0];
    var name = react_1.useState(edit.task.name)[0];
    var _a = react_1.useState(edit.task.description), desc = _a[0], setDesc = _a[1];
    ...
    function changeTasks(tasks, id, name, desc, stat, img, steps, diff, num, time) {
        ...
        edit.updateTasks(copy.map(function (TASK) {
            return TASK.id === id
                ? TaskFunctions_1.makeTask(id, name, desc, stat, img, steps, diff, num, time)
